...
const updateHotspotAttributes = async (hotspotId, description, status, newInfo) => {
  try {
    ...
    const panorama = result.tour.panorama.find(p => p.$.id === 'node21');
    ...
    if (panorama) {
      if (panorama.hotspots && panorama.hotspots.length > 0) {
        ...
        console.warn('Panorama with id "node21" has no hotspots');
        throw new Error(`Panorama with id "node21" has no hotspots`);
      }
    } else {
      console.warn('Panorama with id "node21" not found');
      throw new Error(`Panorama with id "node21" not found`);
    }
  } catch (error) {
    console.error(error);
    return error.message;
